...
import { clamp, map, preloadFonts, preloadImages, randomNumber } from '../utils';
...
lscroll.on('scroll', (obj) => {
    for (const key of Object.keys(obj.currentElements)) {
        ...
        if ( obj.currentElements[key].el.classList.contains('gallery__item') ) {
            ...
            const translationVal = progress > 0.6 ? clamp(map(progress,0.6,1,0,translationArr[idx]), Math.min(0,translationArr[idx]), Math.max(0,translationArr[idx])) : 1;
